...
1. 当当前目录中的步骤上下文不完整、需要刷新，或者需要重新拉取用例步骤详情时，先调用既有步骤获取脚本。
2. 使用既有脚本拉取当前 `case_id` 对应的步骤详情，不自行发明新的查询方式：

```bash
python scripts/fetch_case_steps.py --case_id "CASE_001"
```

3. 将获取到的步骤详情作为后续脚本预处理和修复诊断的输入。
4. 如果当前已经具备完整的 `[case_id]_AI_create.txt` 且上下文足够，则可跳过该步骤。
