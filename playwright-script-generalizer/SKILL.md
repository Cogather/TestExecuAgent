...
**脚本路径约定**：`extract_playwright_params.py`、`restore_playwright_script.py`、`script_to_json_string.py` 与 `param_metadata.py` 位于**同一目录**（与本 SKILL 同属该技能包）。以下命令在**该目录为当前工作目录**时可直接使用文件名；否则请写成指向这些脚本的相对或绝对路径。**文档示例不写出编辑器私有目录（如 `.cursor/`）**，便于拷贝到其它环境使用。
...
1. **运行提取脚本**（推荐每次输出到**独立目录**，便于版本与分享）：
   ```bash
   python extract_playwright_params.py -i <input.py> -d <输出目录> -o <模板文件名.py> -p <参数文件名.json>
   ```
...
**代理应执行**（在工具脚本所在目录下，或把路径补全为指向 `extract_playwright_params.py`；独立目录 + 默认文件名示例）：
`python extract_playwright_params.py -i dir1/login.py -d dir2/login_extract -o template_login.py -p default_params.json`

**用户**：「用 new_env.json 还原 dir2 里的模板」

**代理应执行**（同上，指向 `restore_playwright_script.py`；结果默认在 `out/restored/`，文件名由模板 `converted_*.py` 或旧 JSON 的 `source_script` 决定）：
`python restore_playwright_script.py -t dir2/converted_login.py -p new_env.json`
...
```bash
python script_to_json_string.py out/sample_extract/converted_sample_script.py
# 或写入文件
python script_to_json_string.py out/sample_extract/converted_sample_script.py -o script_json_value.txt
```
